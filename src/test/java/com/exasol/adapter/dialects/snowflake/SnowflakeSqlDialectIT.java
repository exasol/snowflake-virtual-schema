...
class SnowflakeSqlDialectIT {
    ...
    static void beforeAll() throws SQLException {
        ...
        // virtualSchemaSnowflakeUppercaseTable = SETUP.createVirtualSchema(SCHEMA_SNOWFLAKE_UPPERCASE_TABLE,
        // Map.of("IGNORE_ERRORS", "SNOWFLAKE_UPPERCASE_TABLES"));
        // virtualSchemaSnowflakePreserveOriginalCase = SETUP.createVirtualSchema(SCHEMA_SNOWFLAKE_UPPERCASE_TABLE,
        // Map.of("SNOWFLAKE_IDENTIFIER_MAPPING", "PRESERVE_ORIGINAL_CASE"));
    ...
    private static void createSnowflakeTestTableAllDataTypes(final Statement statementSnowflake) throws SQLException {
        ...
        statementSnowflake.execute("CREATE TABLE " + qualifiedTableName + " (" //
                + "myBigint BIGINT,	" //
                ...
                + "myXml XML " //
                + ")");
        statementSnowflake.execute("INSERT INTO " + qualifiedTableName + " VALUES (" //
    ...
    private static void createSnowflakeTestTableMixedCase(final Statement statementSnowflake) throws SQLException {
        final String qualifiedTableName = SCHEMA_SNOWFLAKE_UPPERCASE_TABLE + ".\"" + TABLE_SNOWFLAKE_MIXED_CASE + "\"";
        statementSnowflake.execute("CREATE TABLE " + qualifiedTableName + " (x INT, \"Y\" INT)");
    }

    private static void createSnowflakeTestTableLowerCase(final Statement statementSnowflake) throws SQLException {
        final String qualifiedTableName = SCHEMA_SNOWFLAKE_UPPERCASE_TABLE + "." + TABLE_SNOWFLAKE_LOWER_CASE;
        statementSnowflake.execute("CREATE TABLE " + qualifiedTableName + " (x INT, y INT)");
    }
    ...
    void testCreateSchemaWithUpperCaseTablesThrowsException() {
        final Exception exception = assertThrows(DatabaseObjectException.class,
                () -> SETUP.createVirtualSchema(SCHEMA_SNOWFLAKE_UPPERCASE_TABLE, Map.of()));
    ...
    void testQueryUpperCaseTableQuotedThrowsException() {
        final String selectStatement = "SELECT x FROM  " + virtualSchemaSnowflakeUppercaseTable.getName() + ".\""
        ...
        assertThat(exception.getMessage(), containsString(".\"" + TABLE_SNOWFLAKE_MIXED_CASE + "\" not found"));
    }
    ...
    void testQueryUpperCaseTableThrowsException() {
        final Exception exception = assertThrows(SQLException.class, () -> statementExasol.execute(
                "SELECT x FROM  " + virtualSchemaSnowflakeUppercaseTable.getName() + "." + TABLE_SNOWFLAKE_MIXED_CASE));
        assertThat(exception.getMessage(), containsString("object " + virtualSchemaSnowflakeUppercaseTable.getName()
                + "." + TABLE_SNOWFLAKE_MIXED_CASE.toUpperCase() + " not found"));
    }

    @Test
    void testQueryLowerCaseTable() throws SQLException {
        final ResultSet result = statementExasol.executeQuery(
                "SELECT x FROM " + virtualSchemaSnowflakeUppercaseTable.getName() + "." + TABLE_SNOWFLAKE_LOWER_CASE);
    ...
    void testUnsetIgnoreUpperCaseTablesAndRefreshThrowsException() throws SQLException {
        statementExasol.execute(
                "ALTER VIRTUAL SCHEMA " + virtualSchemaSnowflakeUppercaseTable.getName() + " set ignore_errors=''");
        statementExasol.execute("ALTER VIRTUAL SCHEMA " + virtualSchemaSnowflakeUppercaseTable.getName()
                + " set SNOWFLAKE_IDENTIFIER_MAPPING = 'CONVERT_TO_UPPER'");
        final Exception exception = assertThrows(SQLException.class, () -> statementExasol
                .execute("ALTER VIRTUAL SCHEMA " + virtualSchemaSnowflakeUppercaseTable.getName() + " REFRESH"));
        assertThat(exception.getMessage(), containsString("E-VSSF-6: Table '" + TABLE_SNOWFLAKE_MIXED_CASE
                + "' cannot be used in virtual schema. Set property 'IGNORE_ERRORS' to 'SNOWFLAKE_UPPERCASE_TABLES' to enforce schema creation."));
    }

    @Test
    void testSetIgnoreUpperCaseTablesAndRefresh() throws SQLException {
        statementExasol.execute("ALTER VIRTUAL SCHEMA " + virtualSchemaSnowflakeUppercaseTable.getName()
                + " set ignore_errors='SNOWFLAKE_UPPERCASE_TABLES'");
        final String refresh_schema_query = "ALTER VIRTUAL SCHEMA " + virtualSchemaSnowflakeUppercaseTable.getName()
                + " REFRESH";
    ...
    void testPreserveCaseQueryLowerCaseTableThrowsException() {
        final SQLException exception = assertThrows(SQLException.class,
                () -> statementExasol.executeQuery("SELECT x FROM  "
                        + virtualSchemaSnowflakePreserveOriginalCase.getName() + "." + TABLE_SNOWFLAKE_LOWER_CASE));
        assertThat(exception.getMessage(),
                containsString("object " + virtualSchemaSnowflakePreserveOriginalCase.getName() + "."
                        + TABLE_SNOWFLAKE_LOWER_CASE.toUpperCase() + " not found"));
    }
    ...
    void testPreserveCaseQueryUpperCaseTableWithQuotes() throws SQLException {
        final ResultSet result = statementExasol.executeQuery("SELECT \"Y\" FROM  "
                + virtualSchemaSnowflakePreserveOriginalCase.getName() + ".\"" + TABLE_SNOWFLAKE_MIXED_CASE + "\"");
        assertThat(result.next(), equalTo(false));
    }
